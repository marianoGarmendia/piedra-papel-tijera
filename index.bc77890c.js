...
t.currentGame.myPlay=0,t.currentGame.computerPlay=0,t.currentGame.playTime=0,t.currentGame.ganador="",t.historyGame=[],g.setState(t),e("/inicio")}),r}(a.currentGame,n,t)}}];function S(e){if(console.log("estoy en gh pages"),H()){let t=e.replace(x,"");for(let e of b)if(e.path.test(t)){let t=e.handle(v);E(t)}}else for(let t of b)if(t.path.test(e)){let e=t.handle(v);E(e)}}function E(e){f?.firstChild?(f.firstChild.remove(),f.appendChild(e)):f?.appendChild(e)}function v(e){if(H()){let t=x+e;history.pushState({},"",t),S(t)}else history.pushState({},"",e),S(e)}function L(){"/"==location.pathname?v("/inicio"):S(location.pathname)}function H(){return location.host.includes("github.io")}window.addEventListener("popstate",()=>{L()}),g.initState(),L(),function(){class e extends HTMLElement{constructor(){super(),this.size=this.getAttribute("size")}connectedCallback(){this.render()}render(){let e=document.createElement("div"),t=document.createElement("style"),n=document.createElement("img"),a=document.createElement("img"),r=document.createElement("img");if(n.id="papel",a.id="piedra",r.id="tijera",e.classList.add("container"),t.textContent=`
            .container{
                display: flex;
...
}();//# sourceMappingURL=index.bc77890c.js.map
